...
const userNamePattern = '[A-Za-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u00FF]+ [A-Za-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u00FF]+';
...
const formatNameInput = (value) => {
    ...
    const words = sanitizedValue.split(' ').filter(Boolean).slice(0, 2);
    const formattedName = words.map(formatNameWord).join(' ');
...
export default function UserEdit({ auth, user, units = [] }) {
    ...
    const initialRole = user.funcao !== undefined && user.funcao !== null ? String(user.funcao) : '5';
    const userUnitIds = user.units && user.units.length
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        funcao: initialRole,
        payment_day: user.payment_day !== undefined && user.payment_day !== null && user.payment_day !== ''
            ? String(user.payment_day)
            : '',
                            ...
                            <div className="mb-4 grid gap-4 md:grid-cols-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        title="Informe nome e sobrenome separados por um único espaço"
                                        value={data.name}
